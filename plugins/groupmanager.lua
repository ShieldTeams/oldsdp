...
local lang = redis:get(hash)
  ...
  save_data(_config.moderation.data, data)
     local groups = 'groups'
      ...
      save_data(_config.moderation.data, data)
 if not lang then
  return '_Group has been removed_'
 else
...
elseif lang then
 return "*فحش دادن در گروه هم اکنون ممنوع است*"
end
else
...
elseif lang then
return "*فحش دادن در گروه ممنوع نمیباشد*"
end
else 
...
return "*فحش دادن در گروه آزاد شد*"
end
end
end
...
local text = string.gsub(text,'10','🔟')
return text
end
--------Mutes---------
--------Mute all--------------------------
...
local function getpro(arg, data)
   if data.photos_[0] then
       ...
       if not lang then
      tdcli.sendMessage(msg.to.id, msg.id_, 1, "⛔ _You Have Not Profile Photo...!_\n\n🆎 _Chat ID_ : `"..msg.to.id.."`\n🆔 _User ID_ : `"..msg.from.id.."`", 1, 'md')
       elseif lang then
...
if not lang then
return "_Message Has Been Pinned_"
elseif lang then
return "*پیام سجاق شد*"
end
...
if not lang then
return "_Pin message has been unpinned_"
elseif lang then
...
if matches[2] == 'cmds' then
			...
			return '_cmds has been unlocked for member_'
			end
			end
end
...
if matches[1]:lower() == 'cmds' and is_owner(msg) then 
		...
		return '_cmds set for owner or higher_' 
		end
		end
		...
		return '_cmds set for moderator or higher_'
		end 
		end 
		...
		return '_cmds set for member or higher_' 
		end 
		end 
		end
   ...
   local lang = redis:get(hash)
    local administration = load_data(_config.moderation.data) 
       ...
       return tdcli.sendMessage(msg.to.id, msg.id, 1, "*ربات سازنده گروه نیست*\n*با دستور* setlink/ *لینک جدیدی برای گروه ثبت کنید*", 1, 'md')
					...
					save_data(_config.moderation.data, administration)
        if not lang then
       return tdcli.sendMessage(msg.to.id, msg.id, 1, "_Newlink Created_", 1, 'md')
        elseif lang then
 ...
 tdcli.exportChatInviteLink(msg.to.id, callback_link, nil)
   ...
   local is_link = msg.text:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.text:match("^([https?://w]*.?t.me/joinchat/%S+)$")
    ...
    if matches[1] == 'linkpv' and is_mod(msg) then
     ...
     if not lang then
     tdcli.sendMessage(user, "", 1, "<i>Group Link "..msg.to.title.." :</i>\n"..linkgp, 1, 'html')
     else
      tdcli.sendMessage(user, "", 1, "<b>لینک گروه "..msg.to.title.." :</b>\n"..linkgp, 1, 'html')
         end
      if not lang then
        return "_Group Link Was Send In Your Private Message_"
       else
    ...
    return "_Group_ *flood* _sensitivity has been set to :_ *[ "..matches[2].." ]*"
     ...
     if not lang then
       ...
       if not lang then
            ...
            if not lang then
					return "_Welcome Message not set_"
             else
...
🔹*!del* `[1-999]` 
🔸_Clear the number of group chats up_
...
🔐*!lock* `[link | tag | edit | arabic | webpage | bots | spam | flood | markdown | mention | pin | fosh | cmds]`
🔸_If This Actions Lock, Bot Check Actions And Delete Them_

🔓*!unlock* `[link | tag | edit | arabic | webpage | bots | spam | flood | markdown | mention | pin | fosh | cmds]`
🔸_If This Actions Unlock, Bot Not Delete Them_

🔇*!mute* `[gif | photo | doc | sticker | keyboard | video | text | fwd | loc | audio | voice | contact | all]`
🔸_If This Actions Lock, Bot Check Actions And Delete Them_

🔈*!unmute* `[gif | photo | doc | sticker | keyboard | video | text | fwd | loc | audio | voice | contact | all]`
🔸_If This Actions Unlock, Bot Not Delete Them_
...
🔹*!del* `[1-999]` 
🔸_پاک کردن تعداد چت های گروه تا حداکثر_
