--Begin Tools.lua By @Shield_Tm
local SUDO = 206596952-- put Your ID here! <===
...
local function chat_list(msg)
    ...
    if not data[tostring(groups)] then
        return '_No groups at the moment_'
    end
    local message = '_List of Groups:_\n*Use #join (ID) to join*\n\n'
...
if is_admin1(tonumber(data.id_)) then
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از قبل ادمین ربات بود*", 0, "md")
      end
   end
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *به مقام ادمین ربات منتصب شد*", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if not is_admin1(data.id_) then
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از قبل ادمین ربات نبود*", 0, "md")
      end
   end
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از مقام ادمین ربات برکنار شد*", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if already_sudo(tonumber(data.id_)) then
  if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از قبل سودو ربات بود*", 0, "md")
      end
   end
  ...
  if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *به مقام سودو ربات منتصب شد*", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
user_name = check_markdown(data.first_name_)
   ...
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از قبل سودو ربات نبود*", 0, "md")
      end
   end
   ...
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id_.."* *از مقام سودو ربات برکنار شد*", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
  ...
  return tdcli.sendMessage(data.chat_id_, "", 0, "_User Not Found_", 0, "md")
      end
   end
end
...
if matches[1] == "clear cache" then
    ...
    return "_All Cache Has Been Cleared_"
   end
...
if is_sudo(msg) then
		...
		if matches[1] == 'leave' and matches[2] then
			if lang then
				tdcli.sendMessage(matches[2], 0, 1, '*ربات با دستور سودو از گروه خارج ش.\nبرای اطلاعات بیشتر با سودو تماس بگیر.*', 1, 'md')
				...
				tdcli.sendMessage(matches[2], 0, 1, '_Robot left the group._\n_For more information contact The SUDO._', 1, 'md')
				tdcli.changeChatMemberStatus(matches[2], our_id, 'Left', dl_cb, nil)
				tdcli.sendMessage(SUDO, msg.id_, 1, '_Robot left from under group successfully:_\n\n`'..matches[2]..'`', 1,'md')
			end
		end
		...
		if string.match(matches[2], '^-%d+$') then
			if tonumber(matches[3]) > 0 and tonumber(matches[3]) < 1001 then
				...
				if lang then
					...
					tdcli.sendMessage(SUDO, 0, 1, '_Recharged successfully in the group:_ `'..matches[2]..'`\n_Expire Date:_ `'..matches[3]..'` *Day(s)*', 1, 'md')
					tdcli.sendMessage(matches[2], 0, 1, '_Robot recharged_ `'..matches[3]..'` *day(s)*\n_For checking expire date, send_ `/check`',1 , 'md')
				end
			else
				if lang then
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				...
				tdcli.sendMessage(SUDO, msg.id_, 1, '_Plan 1 Successfully Activated!\nThis group recharged with plan 1 for 30 days (1 Month)_', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_Successfully recharged_\n_Expire Date:_ `30` *Days (1 Month)*', 1, 'md')
			end
		end
		end
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				...
				tdcli.sendMessage(SUDO, msg.id_, 1, '_Plan 2 Successfully Activated!\nThis group recharged with plan 2 for 90 days (3 Month)_', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_Successfully recharged_\n_Expire Date:_ `90` _Days (3 Months)_', 1, 'md')
			end
		end
		end
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				...
				tdcli.sendMessage(SUDO, msg.id_, 1, '_Plan 3 Successfully Activated!\nThis group recharged with plan 3 for unlimited_', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_Successfully recharged_\n_Expire Date:_ `Unlimited`', 1, 'md')
			end
		end
		end
		...
		if string.match(matches[2], '^-%d+$') then
			if lang then
				...
				tdcli.sendMessage(matches[2], 0, 1, '*سودو به گروه اضافه شد.*', 1, 'md')
			else
				tdcli.sendMessage(SUDO, msg.id_, 1, '_I added you to this group:_\n\n`'..matches[2]..'`', 1, 'md')
				tdcli.addChatMember(matches[2], SUDO, 0, dl_cb, nil)
				tdcli.sendMessage(matches[2], 0, 1, '_Sudo Joined!_', 1, 'md')
			end
		end
		end
end
                        ...
                        if data.content_.ID == 'MessageDocument' then
                            ...
                            if file_exi(filename, cpath) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>File</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
								else
						...
						if data.content_.ID == 'MessagePhoto' then
                            ...
                            if file_exi(file..'_(1).jpg', cpath) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Photo</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
		                ...
		                if data.content_.ID == 'MessageSticker' then
                            ...
                            if file_exi(name, secp) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Sticker</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
						...
						local cpath = tcpath..'/data/audio'
							if file_exi(audio_name, cpath) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Audio</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
							else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
						...
						if data.content_.ID == 'MessageVoice' then
                            ...
                            if file_exi(name, secp) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Voice</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
						...
						if data.content_.ID == 'MessageVideo' then
                            ...
                            if file_exi(name, secp) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Video</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
						...
						if data.content_.ID == 'MessageAnimation' then
                            ...
                            if file_exi(anim_name, cpath) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Gif</i> <code>'..folder..'</code> <i>Has Been Saved.</i>', 1, 'html')
								end
                            else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '*فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید.*', 1, 'md')
	...
	if msg.to.type == 'channel' or msg.to.type == 'chat' then
		...
		if matches[1]:lower() == 'check' and is_mod(msg) and not matches[2] then
			...
			if expi == -1 then
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '*گروه به صورت نامحدود شارژ میباشد!*', 1, 'md')
				else
				...
				if lang then
					...
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '`'..day..'` _Day(s) remaining until Expire._', 1, 'md')
				end
			end
		end
...
tdcli.changeUsername(matches[2])
   if not lang then
return '_Bot Username Changed To:_\n@'..matches[2]..'*'
  else
...
1, nil, send_file, '_🆔 Channel ™ :_ @ShieldTeamS', dl_cb, nil)
	end
	if matches[1]:lower() == "sendplug" and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 
1, nil, plug, '🆔 _Channel ™ :_ @ShieldTeamS', dl_cb, nil)
    end
  end
                    ...
                    if data.content_.ID == 'MessageDocument' then
                        ...
                        if (filename:lower():match('.lua$')) then
                            ...
                            if pl_exi(filename) then
                                ...
                                tdcli.sendMessage(msg.to.id, msg.id_,1, '<i>Plugin</i> <code>'..matches[2]..'</code> <i>Has Been Saved.</i>', 1, 'html')
                            else
                                tdcli.sendMessage(msg.to.id, msg.id_, 1, '_This file does not exist. Send file again._', 1, 'md')
                            end
                        else
...
return chat_list(msg)
   ...
   if matches[1]:lower() == 'join' and is_admin(msg) and matches[2] then
	   tdcli.sendMessage(msg.to.id, msg.id, 1, '_I Invite you in_ '..matches[2]..'', 1, 'html')
	   tdcli.sendMessage(matches[2], 0, 1, "_Admin Joined!_", 1, 'html')
    ...
    local data = load_data(_config.moderation.data)
			...
			save_data(_config.moderation.data, data)
	   tdcli.sendMessage(matches[2], 0, 1, "_Group has been removed by admin command_", 1, 'html')
    return '_Group_ *'..matches[2]..'* _removed_'
		end
if matches[1] == 'shield' then
...
--Enable Auto Leave
    ...
    redis:del(hash)
   return '_Auto leave has been enabled_'
--Disable Auto Leave
     elseif matches[2] == 'disable' then
    redis:set(hash, true)
   return '_Auto leave has been disabled_'
--Auto Leave Status
      elseif matches[2] == 'status' then
      if not redis:get(hash) then
   return '_Auto leave is enable_'
       else
   return '_Auto leave is disable_'
         end
      end
   end
...
🌟 _Sudoer And Admins Shield Power Bot Help :_ 🌟
...
text = [[
🌟 _راهنمای ادمین و سودو های ربات شیلد پاور:_ 🌟
