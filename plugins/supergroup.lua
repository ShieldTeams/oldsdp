...
local title ="<b>Info for SuperGroup</b> : <code>["..result.title.."]</code>\n\n"
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return '<b>Forward posting is already locked</b>'
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<b>Forward posting has been locked</b>'
  end
end
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return '<b>Forward is not locked</b>'
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'no'
    save_data(_config.moderation.data, data)
    return '<b>Forward has been unlocked</b>'
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return '<b>Reply posting is already locked</b>'
  else
    data[tostring(target)]['settings']['lock_reply'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<b>Reply posting has been locked</b>'
  end
end
...
if is_muted(tostring(target), 'Audio: yes') then
 ...
 All = 'no'
  ...
  local text = "___________________________\n___________________________\n\n⚙§u℘εƦ ❡Ʀøu℘ §ε††ιπ❡$⚙\n___________________________\n___________________________\n\n🌐 <b>Lock links</b> ➣ ["..settings.lock_link.."]\n📲 <b>Lock contacts</b> ➣ ["..settings.lock_contacts.."]\n📝 <b>Lock flood</b> ➣ ["..settings.flood.."]\n🆘 <b>Counter security</b> ➣《"..NUM_MSG_MAX.."》\n📩 <b>Lock spam</b> ➣ ["..settings.lock_spam.."]\n☫ <b>Lock arabic</b> ➣ ["..settings.lock_arabic.."]\n🏃 <b>Lock member</b> ➣ ["..settings.lock_member.."]\n🔙 <b>Lock RTL</b> ➣ ["..settings.lock_rtl.."]\n🚻 <b>Lock tgservice</b> ➣ ["..settings.lock_tgservice.."]\n🎭 <b>Lock sticker</b> ➣ ["..settings.lock_sticker.."]\n⚠️ <b>Lock tag</b> ➣ ["..settings.tag.."]\n😂 <b>Lock emoji</b> ➣ ["..settings.emoji.."]\n🔠 <b>Lock english</b> ➣ ["..settings.english.."]\n🔂 <b>Lock forward</b> ➣ ["..settings.lock_fwd.."]\n✂ <b>Lock reply</b> ➣ ["..settings.lock_reply.."]\n🚹 <b>Lock join</b> ➣ ["..settings.join.."]\n🆔 <b>Lock username</b> ➣ ["..settings.username.."]\n📺 <b>Lock media</b> ➣ ["..settings.media.."]\n😠 <b>Lock fosh</b> ➣ ["..settings.fosh.."]\n⚡ <b>Lock leave</b> ➣ ["..settings.leave.."]\n👾 <b>Lock bots</b> ➣ ["..settings.lock_bots.."]\n📡 <b>Lock operator</b> ➣ ["..settings.operator.."]\n___________________________\n___________________________\n\n⬇εαšψ šω & ƒαš†εr šω️⬇\n___________________________\n___________________________\n\n⛓ ŁФȻҚ #Etehad ➣ ["..settings.etehad.."]\n✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠✠\n🔑 ŁФȻҚ #All ➣ ["..settings.all.."]\n___________________________\n___________________________\n\n⬇️🔊ﾚïš† ƒ⊕ℜ ℳʊ†Є$🚫⬇️️\n___________________________\n___________________________\n\n🔇 ℳʊ†Є #File ▶《"..Documents.."‌》\n🔇 ℳʊ†Є #Gifs ▶《"..Gifs.."》\n🔇 ℳʊ†Є #Video ▶《"..Video.."》\n🔇 ℳʊ†Є #Photo ▶《"..Photo.."》\n🔇 ℳʊ†Є #Text ▶《"..Text.."》\n🔇 ℳʊ†Є #Audio ▶《"..Audio.."》\n🔕 ℳʊ†Є #All ▶《"..All.."》\n___________________________\n___________________________\n\n️⬇️ℹ️️ΛɓøŪŦ ɢŔøŪṖℹ️⬇️\n___________________________\n___________________________\n\n👥#ɢяøʊρ †૪Ƿє : "..gp_type.."\n➖➖➖➖➖➖➖➖➖➖➖\n⭐ yes øℜ no #Public ➣ ["..settings.public.."]\n➖➖➖➖➖➖➖➖➖➖➖\n📛 $†Ʀιc† #Settings ➣ ["..settings.strict.."]\n___________________________\n___________________________\n\n➣ ♈️️️вΦƮ vęЯsiΦП ••• V 2.0♈️\n\n➣ 👑SHIELD™POWER👑\n\n☞☞🆔 @Shield_Tm 🆔☜☜\n\n_🇸__🇭__🇮__🇪__🇱__🇩_\n\n▶#Requester username : [@"..(msg.from.username or "----").."]\n▶#Requester ID:["..msg.from.id.."]\n\n🇸 🇭 🇮 🇪 🇱 🇩™"
...
local function demote2(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver,member_username..' <b>is not a moderator.</b>')
  end
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "invite" then
	...
	elseif get_cmd == "setowner" then
		...
		if result.username then
			text = member_username.." <code>[ "..result.peer_id.." ]</code> <b>added as owner</b>"
		else
			text = "<code>[ "..result.peer_id.." ]</code> <b>added as owner</b>"
		end
		send_large_msg(receiver, text)
  end]]
	elseif get_cmd == "promote" then
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." <code>["..v.peer_id.."]</code> <b>added as owner</b>"
				else
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "➣ <b>Your Name</b> : <code>" ..string.gsub(msg.from.print_name, "_", " ").. "</code>\n\n➣ <b>Your Username</b> : @"..(msg.from.username or '----').."\n➣ <b>Your ID</b> : <code>" ..msg.from.id.. "</code>\n\n➣ <b>SuperGroup Name</b> : <code>" ..string.gsub(msg.to.print_name, "_", " ").. "</code>\n\n➣ <b>SuperGroup ID</b> : <code>"..msg.to.id.."</code>"
			end
		end
		...
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					...
					local text = "<code>[ "..matches[2].." ]</code> <b>added as owner</b>"
					return text
				end]]
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "<b>"..msg_type.." has been muted</b>"
				else
			...
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "<b>"..msg_type.." have been muted</b>"
				else
					return "<b>SuperGroup mute "..msg_type.." is already on</b>"
				end
			end
		...
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "<b>"..msg_type.." has been unmuted</b>"
				else
			...
			local msg_type = 'Gifs'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "<b>"..msg_type.." have been unmuted</b>"
				else
					return "<b>Mute "..msg_type.." is already off</b>"
				end
			end
