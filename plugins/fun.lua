...
--------------------------------
	if matches[1]:lower() == 'praytime' or matches[1] == 'azan' then
		...
		local text = '🛣 شهر : '..city
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n🙉 اذان صبح : '..data.Fajr
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n☀️  طلوع آفتاب : '..data.Sunrise
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n🙉 اذان ظهر : '..data.Dhuhr
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n🌅  غروب آفتاب : '..data.Sunset
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n🙉 اذان مغرب : '..data.Maghrib
	  text = text..'\n➖➖➖➖➖➖➖➖➖➖➖\n🙉 عشاء : '..data.Isha
	  text = text..'\n\n🌔🌜🌕🌞⭐️🌞⭐️🌞🌕🌛🌖\n\n🆔 _Channel ™ :_ @ShieldTea4mS\n=======================\n🇸 🇭 🇮 🇪 🇱 🇩™\n'
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "🆔 _Channel ™ :_ @ShieldTeamS", dl_cb, nil)
					else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '🆔 _Channel ™ :_ @ShieldTeamS', dl_cb, nil)
					else
...
if matches[1] == 'voice' then
 local text = matches[2]
  ...
  local file = download_to_file(url,'SDP_Team.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '🆔 _Channel ™ :_ @ShieldTeamS', dl_cb, nil)
   end
end
...
--------------------------------
	if matches[1]:lower() == "photo" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "🆔 _Channel ™ :_ @ShieldTeamS", dl_cb, nil)
	end
